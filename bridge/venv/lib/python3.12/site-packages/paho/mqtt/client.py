...
CONNACK_REFUSED_BAD_USERNAME_PASSWORD = <REDACTED>
...
def connack_string(connack_code: int|ReasonCode) -> str:
    ...
    elif connack_code == CONNACK_REFUSED_BAD_USERNAME_PASSWORD:<REDACTED>
...
def convert_connack_rc_to_reason_code(connack_code: ConnackCode) -> ReasonCode:
    ...
    if connack_code == ConnackCode.CONNACK_REFUSED_BAD_USERNAME_PASSWORD:<REDACTED>
...
class Client:
    ...
    * Use `disconnect()` to disconnect from the broker
    ...
    def password(self, value: str | None) -> None:
        if not self._connection_closed():
            raise RuntimeError("updating password <REDACTED>
    ...
    def tls_set(
        ...
        :param str certfile: PEM encoded client certificate filename. Used with
            ...
            broker dependent. Note that if the files in encrypted and needs a password <REDACTED>
            decrypt it, then this can be passed using the keyfile_password <REDACTED>
            should take precautions to ensure that your password <REDACTED>
            not hard coded into your program by loading the password <REDACTED>
            for example. If you do not provide keyfile_password, the password <REDACTED>
    ...
    def username_pw_set(
        self, username: str | None, password: <REDACTED>
    ) -> None:
        """Set a username and optionally a password <REDACTED>
        ...
        :param str username: The username to authenticate with. Need have no relationship to the client id. Must be str
            [MQTT-3.1.3-11].
            Set to None to reset client back to not using username/password <REDACTED>
        :param str password: <REDACTED>
...
class _WebsocketWrapper:
    ...
    def _do_handshake(self, extra_headers: WebSocketHeaders | None) -> None:
                ...
                if len(self._readbuffer) > 2:
                    ...
                    if b"sec-websocket-accept" in str(self._readbuffer).lower().encode('utf-8'):
                        ...
                        if server_hash != client_hash:
                            raise WebsocketConnectionError(
                                "WebSocket handshake error, invalid secret <REDACTED>
                        else:
                            has_secret = <REDACTED>
                else:
                    # ending linebreak
        ...
        if not has_upgrade or not has_secret:<REDACTED>
            raise WebsocketConnectionError("WebSocket handshake error")
