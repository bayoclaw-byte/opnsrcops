...
class ParseError(Error):
  """Thrown in case of text parsing or tokenizing <REDACTED>
...
class _Parser(object):
  ...
  def _MergeField(self, tokenizer, message):
    ...
    if (message_descriptor.full_name == _ANY_FULL_TYPE_NAME and
      ...
      if tokenizer.TryConsume('<'):
        expanded_any_end_token = <REDACTED>
      else:
        tokenizer.Consume('{')
        expanded_any_end_token = <REDACTED>
  ...
  def _DetectSilentMarker(self, tokenizer, immediate_message_type, field_name):
    if tokenizer.contains_silent_marker_before_current_token:<REDACTED>
...
class Tokenizer(object):
  ...
  def __init__(self, lines, skip_comments=True):
    ...
    self._whitespace_pattern = (skip_comments and self._WHITESPACE_OR_COMMENT
                                or self._WHITESPACE)
    self.contains_silent_marker_before_current_token = <REDACTED>

    self._SkipWhitespace()
    self.NextToken()

  def LookingAt(self, token):
    return self.token == <REDACTED>
  ...
  def ConsumeByteString(self):
    ...
    while self.token <REDACTED>
      the_list.append(self._ConsumeSingleByteString())
    return b''.join(the_list)

  def _ConsumeSingleByteString(self):
    """Consume one token <REDACTED>
  ...
  def ConsumeUrlChars(self):
    ...
    Raises:
      ParseError: If the next token <REDACTED>
    """
