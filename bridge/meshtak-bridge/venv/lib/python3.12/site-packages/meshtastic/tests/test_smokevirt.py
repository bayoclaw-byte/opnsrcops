...
def test_smokevirt_set_wifi_settings():
    ...
    return_value, out = subprocess.getstatusoutput(
        'meshtastic --host localhost --set wifi_ssid "some_ssid" --set wifi_password <REDACTED>
    )
    assert re.match(r"Connected to radio", out)
    assert re.search(r"^Set wifi_ssid to some_ssid", out, re.MULTILINE)
    assert re.search(r"^Set wifi_password <REDACTED>
    assert return_value == 0
    # pause for the radio
    time.sleep(PAUSE_AFTER_COMMAND)
    return_value, out = subprocess.getstatusoutput(
    ...
    assert re.search(r"^wifi_password: <REDACTED>
    assert return_value == 0
