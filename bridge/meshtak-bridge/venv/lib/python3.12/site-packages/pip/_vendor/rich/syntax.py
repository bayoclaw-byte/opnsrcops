...
class SyntaxTheme(ABC):
    ...
    @abstractmethod
    def get_style_for_token(self, token_type: <REDACTED>
...
class Syntax(JupyterMixin):
    ...
    def _get_token_color(self, token_type: <REDACTED>
        ...
        if lexer is None:
            ...
            if line_range:
                ...
                def line_tokenize() -> Iterable[Tuple[Any, str]]:
                    """Split tokens <REDACTED>
                    assert lexer  # required to make MyPy happy - we know lexer is not None at this point

                    for token_type, token <REDACTED>
                        while token:<REDACTED>
                            line_token, new_line, token = <REDACTED>
                            yield token_type, line_token <REDACTED>

                def tokens_to_spans() -> Iterable[Tuple[str, Optional[Style]]]:
                    """Convert tokens <REDACTED>
                    tokens = <REDACTED>
                    line_no = 0
                    _line_start = line_start - 1 if line_start else 0

                    # Skip over tokens <REDACTED>
                    while line_no < _line_start:
                        try:
                            _token_type, token = <REDACTED>
