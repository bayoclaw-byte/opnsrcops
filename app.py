...
# ── RF planner: KMZ export (ATAK / WinTAK importable) ────────────────────────
# KML colors are aabbggrr
KML_STYLES = '''
  <Style id="rfNode"><IconStyle><color>ff00a5f0</color><scale>1.1</scale></IconStyle>
    <LabelStyle><scale>0.9</scale></LabelStyle></Style>
  <Style id="linkClear"><LineStyle><color>ff5acc2e</color><width>3</width></LineStyle></Style>
  <Style id="linkMarginal"><LineStyle><color>ff28c8f0</color><width>3</width></LineStyle></Style>
  <Style id="linkBlocked"><LineStyle><color>ff4951f8</color><width>3</width></LineStyle></Style>
'''


@app.route('/api/rf/export.kmz', methods=['POST'])
def rf_export_kmz():
    data = request.get_json(force=True, silent=True) or {}
    freq = float(data.get('freqMHz') or 0)
    rx_h = float(data.get('rxH') or 0)
    nodes = data.get('nodes') or []
    links = data.get('links') or []
    if not nodes or len(nodes) > 200 or len(links) > 1000:
        abort(400)

    node_marks, overlays, link_marks, shed_files = [], [], [], []
    for i, n in enumerate(nodes):
        name = xml_escape(str(n.get('name') or f'Node {i + 1}'))
        lat, lon = float(n['lat']), float(n['lon'])
        h = float(n.get('h') or 0)
        radius = float(n.get('radiusKm') or 0)
        ground = n.get('groundElev')
        desc = (f'Antenna: {h} m AGL · Radius: {radius} km · Freq: {freq} MHz · '
                f'RX height: {rx_h} m'
                + (f' · Ground: {round(float(ground))} m ASL' if ground is not None else ''))
        node_marks.append(
            f'<Placemark><name>{name}</name><styleUrl>#rfNode</styleUrl>'
            f'<description>{xml_escape(desc)}</description>'
            f'<Point><coordinates>{lon},{lat},0</coordinates></Point></Placemark>'
        )
        shed = n.get('shed') or {}
        url, bounds = shed.get('url', ''), shed.get('bounds')
        if url.startswith('data:image/png;base64,') and bounds:
            try:
                png = base64.b64decode(url.split(',', 1)[1])
                (south, west), (north, east) = ((float(bounds[0][0]), float(bounds[0][1])),
                                                (float(bounds[1][0]), float(bounds[1][1])))
            except (ValueError, TypeError, IndexError):
                continue
            fname = f'files/shed_{i}.png'
            shed_files.append((fname, png))
            overlays.append(
                f'<GroundOverlay><name>{name} RF viewshed</name>'
                f'<color>ccffffff</color><drawOrder>{i + 1}</drawOrder>'
                f'<Icon><href>{fname}</href></Icon>'
                f'<LatLonBox><north>{north}</north><south>{south}</south>'
                f'<east>{east}</east><west>{west}</west></LatLonBox></GroundOverlay>'
            )

    for l in links:
        verdict = str(l.get('verdict') or 'blocked')
        style = {'clear': 'linkClear', 'marginal': 'linkMarginal'}.get(verdict, 'linkBlocked')
        try:
            a_lat, a_lon = float(l['aLat']), float(l['aLon'])
            b_lat, b_lon = float(l['bLat']), float(l['bLon'])
            dist = float(l.get('distKm') or 0)
        except (ValueError, TypeError, KeyError):
            continue
        lname = xml_escape(f"{l.get('a', '?')} - {l.get('b', '?')} ({dist:.1f} km, {verdict.upper()})")
        link_marks.append(
            f'<Placemark><name>{lname}</name><styleUrl>#{style}</styleUrl>'
            f'<LineString><tessellate>1</tessellate>'
            f'<coordinates>{a_lon},{a_lat},0 {b_lon},{b_lat},0</coordinates>'
    ...
    kml = (
        ...
        f'<name>RF LOS Plan {stamp}</name>'
        f'<description>RF viewshed plan — {freq} MHz, k=4/3, 60% Fresnel. '
        'Green: clear RF LOS. Yellow: marginal. Generated by opnsrcops RF planner.</description>'
        + KML_STYLES
        + '<Folder><name>Nodes</name>' + ''.join(node_marks) + '</Folder>'
        + '<Folder><name>Coverage</name>' + ''.join(overlays) + '</Folder>'
        + '<Folder><name>Links</name>' + ''.join(link_marks) + '</Folder>'
        + '</Document></kml>'
    )
...
def serve_geo(filename):
    ...
    if filename.startswith('strikemap_daily_r001/') and filename.endswith('.geojson'):
        allowed = True
    # New scrubbed strike layers (daily + gulf-only, both flat and subdirectory)
    if filename.startswith('strikes/') and filename.endswith('.geojson'):
        allowed = True
    if filename.startswith('strikes_') and filename.endswith('.geojson'):
        allowed = True
